...
for k=1:Nmax
    ...
    for i=1:N
        s =0;
	    for j=1:N
	       if i ~= j, s = s + A(i,j)*xo(j,1); 
           end  
    end
        xp(i,1) = (-s + B(i,1))/A(i,i);        
    end
    jerror(k) = norm(A*xp-B,inf);
    if jerror(k) < TOL, break; end
    xo=xp;
    if k == Nmax
...
tjac=toc; % timing
    fprintf(' \n \n time=%f, It. Num. = %3.0f, error = %15.13e\n\n\n', ...
             tjac,k, jerror(k))        
semilogy(1:counts,jerror(1:counts));
grid on
xlabel('iteration number')
ylabel('L_\infty error')
title('JACOBI ITERATION TECHNIQUE')
